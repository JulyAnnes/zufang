        ...
        <TR>
          ...
          <TD vAlign=top>
            ...
            background=qtimages/zjgdj_79.gif border=0>
              ...
              房源名称:<input name="mingchen"  type="text" value="${mingchen }"/> 
                    ...
                    <TR>
                      <TD style="PADDING-TOP: 8px" align=middle class=newsline><table width="98%" border="0" align="center" cellpadding="3" cellspacing="1" bordercolor="#FA9090" style="border-collapse:collapse">
                        ...
                        <tr>
                         <td >${bean.xingming }</td>
                         <td >${bean.tel }</td>
                          <td >${bean.house.bianhao }</td>
             
                          <td >${bean.house.mingchen }</td>
                          <td >${bean.house.money }</td>
                          <td >${bean.house.mianji }</td>
                          <td >${bean.status }</td>
                          <td >${bean.ctime }</td>
                         
                          <td >
                           <a href="indexmethod!zufangupdate3?id=${bean.id }" >查看详情</a>
                         
                           <c:if test="${bean.status=='预约中'}">
                          <a href="indexmethod!zufangdelete?id=${bean.id }" onclick="{if(confirm('确认删除?')){return true;}return false;}">删除</a>
                          
                          
                          </c:if>
                          
                          <c:if test="${bean.status=='租房失败'}">
                          <a href="indexmethod!zufangdelete?id=${bean.id }" onclick="{if(confirm('确认删除?')){return true;}return false;}">删除</a>
                          
                          
                          </c:if>
                          
                          <c:if test="${bean.status=='已同意待支付'}">
                          <a href="indexmethod!zufangupdate5?id=${bean.id }" >支付</a>
