                    ...
                    <TR>
                      <TD style="PADDING-TOP: 8px" align=middle class=newsline><table width="98%" border="0" align="center" cellpadding="3" cellspacing="1" bordercolor="#FA9090" style="border-collapse:collapse">
                        ...
                        <tr>
                          ...
                          <td >
                           <a href="indexmethod!houseupdate3?id=${bean.id }" >查看详情</a>
                         
                           <c:if test="${bean.status=='审核中'}">
                          <a href="indexmethod!housedelete?id=${bean.id }" onclick="{if(confirm('确认删除?')){return true;}return false;}">删除</a>
                          
                          <a href="indexmethod!houseupdate?id=${bean.id }" >修改</a>
                          </c:if>
                          
                          <c:if test="${bean.status=='审核失败'}">
                          <a href="indexmethod!housedelete?id=${bean.id }" onclick="{if(confirm('确认删除?')){return true;}return false;}">删除</a>
                          ...
                          <c:if test="${bean.status=='租赁中'}">
                          
                          <a href="indexmethod!housedelete2?id=${bean.id }" onclick="{if(confirm('确认取消?')){return true;}return false;}">取消租赁</a>
